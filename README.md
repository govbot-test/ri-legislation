...
Formatted, versioned legislative data for Rhode Island. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Rhode Island — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/ri-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`ri-legislation`) — reads the scraper repo's output and:
...
Current settings for Rhode Island:

- **State code**: `ri`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/ri-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:ri/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:ri/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:ri/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:ri/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/ri-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
